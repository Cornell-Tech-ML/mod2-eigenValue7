...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        """
        (t1,) = ctx.saved_values
        return grad_output.f.mul_zip(t1.f.add_zip(t1, t1.f.neg_map(t1.f.mul_zip(t1, t1))), grad_output)
...
class Sum(Function):
    ...
    def forward(ctx: Context, t1: Tensor, dim: Tensor) -> Tensor:
        ...
        Returns:
            Tensor: The result of the reduction operation.
        """
        ctx.save_for_backward(t1._tensor.shape)
        if int(dim.item()) == -1:
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return grad_output.f.add_zip(zeros(shape), grad_output), 0
...
class Permute(Function):
    ...
    def forward(ctx: Context, t1: Tensor, dim: Tensor) -> Tensor:
        ...
        """
        order = dim._tensor._storage
        order = [int(o) for o in order]
        # print("function",order)
        ctx.save_for_backward(order)
        t1._tensor = t1._tensor.permute(*order)
        # print("what",t1.shape)
        return t1
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        grad_output._tensor = grad_output._tensor.permute(*reverse_order)
